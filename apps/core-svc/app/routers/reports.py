...
from app.scorecard import ensure_schema
        ...
        if project_slug:
            if entity_id:
                sql = """
                    ...
                    JOIN entity_projects p
                      ON p.name = v.project_name
                    WHERE p.slug = $1 AND p.entity_id = $2
                    ORDER BY
                        v.project_name,
                        v.pillar_name,
                        v.kpi_name;
                """
                rows = await conn.fetch(sql, project_slug, entity_id)
            else:
                sql = """
                    SELECT
                        v.project_name,
...
async def get_projects_pillars_kpis_report(
    ...
    """
    try:
        rows = await _fetch_project_pillar_kpi_rows(project_slug=project_slug, entity_id=entity_id)
