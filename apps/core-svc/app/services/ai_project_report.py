...
def build_ai_project_report(
    ...
    overall_score_effective = (
        float(overall_score_override)
        if overall_score_override is not None
        else float(overall_score)
    )
    overall_band, overall_icon = get_score_band(overall_score_effective)
    ...
    md.append("")
    md.append(
        f"Overall trust & governance posture: {overall_score_effective:.1f}% "
