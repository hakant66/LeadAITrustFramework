// apps/web/src/app/scorecard/[projectId]/vipdashboard/page.tsx (and dashboard)
...
export default async function ScorecardDashboard(
  ...
  if (res.status === 410) {
    return (
      <ArchivedProjectNotice
        projectId={projectId}
        subtitle="LeadAI · Project Trust Score"
      />
    );
  }
  ...
  // 3) Fetch canonical pillars via app API so session is forwarded
...
function TrustMarkBadge({
  ...
  return (
    <Link href={link} className="inline-block">
      <div
        className="w-full max-w-[280px] rounded-2xl p-2 bg-white dark:bg-slate-900"
        style={{ maxWidth: size + 16 }}
      >
        <div
          className="relative w-full overflow-hidden rounded-xl ring-1 ring-gray-200 dark:ring-slate-700 bg-white dark:bg-slate-950"
          style={{ maxWidth: size }}
        >
          ...
          <Image
            ...
            fill
            sizes="(max-width: 640px) 70vw, (max-width: 1024px) 280px, 220px"
