...
type KpiDetail = {
  ...
  euaiact_chapter?: string | null;
  euaiact_section?: string | null;
...
async function fetchJsonOk<T = any>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (res.status === 410) throw new Error("PROJECT_ARCHIVED");
...
export default function KpiDetailPage() {
  ...
  useEffect(() => {
    ...
    fetchJsonOk<KpiDetail>(
      `/api/core/scorecard/${encodeURIComponent(
        projectId,
      )}/kpis/${encodeURIComponent(kpiKey)}?entity_id=${encodeURIComponent(entityId)}`,
      { cache: "no-store" },
    )
      .then((result) => {
        if (!cancelled) {
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-gray-50 text-slate-900 dark:bg-slate-950 dark:text-slate-50">
        <div className="px-6 py-6 max-w-7xl mx-auto">Loading KPI…</div>
      </main>
    );
  }

  if (error || !data) {
    if (error && error.includes("PROJECT_ARCHIVED")) {
      return (
        <ArchivedProjectNotice
          projectId={projectId}
          subtitle="LeadAI · KPI Detail"
        />
      );
    }
    return (
      ...
      <div className="px-6 py-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            ...
            <p className="whitespace-pre-line text-sm text-slate-700 dark:text-slate-200">
              {data.euaiact_clause || data.eu_ai_act_clause || "—"}
            </p>
          </div>
