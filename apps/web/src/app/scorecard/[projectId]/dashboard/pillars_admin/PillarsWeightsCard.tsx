...
export default function PillarsWeightsCard({
  ...
  const targetProjects = useMemo(
    () =>
      Array.from(new Set((projects && projects.length ? projects : project ? [project] : []).filter(Boolean))),
    [project, projects],
  );
    ...
    try {
      ...
      for (const target of targetProjects) {
        const res = await fetch(
          `/api/scorecard/${encodeURIComponent(target)}/pillar_weights`,
          {
            method: "PUT",
        ...
        if (!res.ok) {
          console.error(`Save failed for ${target}`, await res.text());
          return;
        }
      }
    ...
    >
      ...
      <p className="text-sm text-slate-600 dark:text-slate-300">
        ...
        {targetProjects.length > 1
          ? ` Changes are applied to all ${targetProjects.length} projects in this entity.`
          : ""}
      </p>
