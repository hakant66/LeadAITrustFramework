...
export default async function PortfolioSummaryRedirectPage() {
  ...
  if (!res.ok) {
    if (res.status === 401) {
      redirect("/register?callbackUrl=" + encodeURIComponent("/scorecard/PortfolioSummary"));
    }
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50">
        ...
        <div className="mx-auto max-w-2xl px-6 py-12">
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 dark:border-amber-800 dark:bg-amber-950/40">
            ...
            <Link
              href="/scorecard/PortfolioSummary"
  ...
  redirect(`/${encodeURIComponent(first.slug)}/scorecard/PortfolioSummary`);
}
