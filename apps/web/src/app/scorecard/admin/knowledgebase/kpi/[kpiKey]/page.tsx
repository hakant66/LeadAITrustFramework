...
export default function KnowledgeBaseKpiPage() {
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-slate-900 dark:bg-slate-950 dark:text-slate-50">
      <div className="px-6 py-6 max-w-6xl mx-auto">
        <Header
          title={data.kpi_name}
          subtitle={`Knowledge Base KPI · ${data.kpi_key.toUpperCase()}`}
        >
          <CloseWindowButton
            className="
      ...
      <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="mt-2 text-xs text-slate-500 dark:text-slate-300">
          Continue to controls, then proceed to the policy register.
        </div>
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <button
            type="button"
            className="rounded-full bg-indigo-600 px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-indigo-500"
