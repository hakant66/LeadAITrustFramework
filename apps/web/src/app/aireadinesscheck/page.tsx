...
import { buildEnablers, type Enabler as EnablerType } from "./seed/enablers";
...
type TotalsRow = { name: string; sum: number; readiness: number; status: StatusKey };
...
const normalizeStatusKey = (status: string | null | undefined): StatusKey => {
  if (!status) return "critical";
  const normalized = status.toLowerCase();
  if (normalized === "critical") return "critical";
  if (normalized === "at risk" || normalized === "atrisk") return "atRisk";
...
function createInitialScores(enablers: EnablerType[]): Record<string, StageScore[]> {
...
function intensityLabelForScore(
  score: StageScore,
  t: (key: string) => string
): string {
  switch (score) {
    case 0:
    case 4:
      return t("intensity.alwaysTrue");
    case 1:
    case 3:
      return t("intensity.sometimesTrue");
    case 2:
    default:
      return t("intensity.neutral");
  }
}
...
function buildAnswerSummary(
  ...
  return enablers.map((enabler) => {
    ...
    const questions = enabler.themes.map((theme, idx) => {
      ...
      const intensity = intensityLabelForScore(score, t);

      let selectionLabel: string;
      let selectionText: string;

      if (alignment === "left") {
        selectionLabel = t("review.selection.leftWithIntensity", {
          intensity,
        });
        selectionText = theme.blue;
      } else if (alignment === "right") {
        selectionLabel = t("review.selection.rightWithIntensity", {
          intensity,
        });
        selectionText = theme.orange;
      } else {
        selectionLabel = t("review.selection.neutralWithIntensity", {
          intensity,
        });
        selectionText =
          t("review.selection.neutralSummary");
      }
...
export default function AIReadinessCheckPage() {
  ...
  const enablers = buildEnablers(t);
  // page:
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed?.totals && typeof parsed?.avg === "number") {
        const normalizedTotals = (parsed.totals as Array<{
          name: string;
          sum: number;
          readiness: number;
          status: string;
        ...
        const timer = window.setTimeout(
          () => setShared({ totals: normalizedTotals, avg: parsed.avg }),
          0
        );
  ...
  async function persistResultsOnServer() {
    ...
    const totalsForServer = totals.map((row) => ({
      ...row,
      status: STATUS_LABELS[row.status],
    }));
    const avg = computeAvg(totals);
    const answers = buildAnswerSummary(scores, enablers, t);
                ...
                >
                  ...
                  <div className="space-y-3">
                    {enabler.themes.map((theme, idx) => {
                      ...
                      if (alignment === "left") {
                        selectionLabel = t("review.selection.left");
                        selectionText = theme.blue;
                      } else if (alignment === "right") {
                        selectionLabel = t("review.selection.right");
                        selectionText = theme.orange;
                      } else {
                        selectionLabel = t("review.selection.neutral");
                        selectionText =
                          t("review.selection.neutralSummary");
                      }
      ...
      <>
        ...
        <Results
          ...
          onRestart={() => {
            setPage(0);
            setHasStarted(false);
            setScores(createInitialScores(enablers));
            setUserInfo({
              firstName: "",
              lastName: "",
              email: "",
