...
type StatusKey = "critical" | "atRisk" | "established" | "leading";
...
function statusBadgeColor(status: StatusKey): string {
  switch (status) {
    case "critical":
...
const statusKeyFromScore = (v: number): StatusKey =>
  v < 25 ? "critical" : v < 50 ? "atRisk" : v < 75 ? "established" : "leading";
...
export default function Results({
  ...
  const t = useTranslations("AiReadiness");
  ...
  const handleDownload = async () => {
    ...
    const drawSummaryAndTable = () => {
      ...
      pdf.setFontSize(12);
      pdf.text(t("pdf.assessmentDetails"), marginX, y);
      ...
      if (userInfo?.company) {
        pdf.text(`${t("pdf.company")}: ${userInfo.company}`, marginX, y);
        y += 6;
      }
      if (userInfo?.firstName || userInfo?.lastName) {
        ...
        if (fullName) {
          pdf.text(`${t("pdf.name")}: ${fullName}`, marginX, y);
          y += 6;
        }
      }
      if (userInfo?.email) {
        pdf.text(`${t("pdf.email")}: ${userInfo.email}`, marginX, y);
        y += 6;
      }
      if (completedAt) {
        ...
        pdf.text(`${t("pdf.dateTime")}: ${dateLine}`, marginX, y);
        y += 10;
      }
      ...
      pdf.setFontSize(12);
      pdf.setTextColor(0, 0, 0);
      pdf.text(t("pdf.overallReadiness"), marginX, y);
      ...
      pdf.setTextColor(0, 0, 0);
      pdf.text(
        `(${t(`status.${statusKeyFromScore(avg)}`)})`,
        marginX + 35,
        y
      );
      ...
      pdf.text(t("pdf.scale"), marginX, y);
      ...
      pdf.text(t("pdf.table.enabler"), colX.enabler, y);
      pdf.text(t("pdf.table.total"), colX.total, y);
      pdf.text(t("pdf.table.readiness"), colX.readiness, y);
      pdf.text(t("pdf.table.status"), colX.status, y);
      ...
      totals.forEach((row) => {
        ...
        const lines = pdf.splitTextToSize(row.name, maxWidth) as string[];

        lines.forEach((ln: string, i: number) => {
          if (i === 0) {
            pdf.text(String(ln), colX.enabler, y);
            pdf.text(String(row.sum), colX.total, y, { align: "left" });
            pdf.text(`${row.readiness}%`, colX.readiness, y, { align: "left" });
            ...
            pdf.text(t(`status.${row.status}`), colX.status, y, {
              align: "left",
            });
            pdf.setTextColor(0, 0, 0);
          } else {
      ...
      pdf.setTextColor(100, 116, 139);
      pdf.setFontSize(9);
      pdf.text(t("pdf.footer"), marginX, footerY);
      pdf.text(t("pdf.copyright"), marginX, footerY + 6);
    };
    ...
    const drawSelectedAnswers = () => {
      ...
      pdf.setFontSize(14);
      pdf.setTextColor(0, 0, 0);
      pdf.text(t("pdf.selectedAnswers"), 15, y);
      y += 8;
      ...
      answers.forEach((block) => {
        ...
        block.questions.forEach((q) => {
          ...
          const leftLines = pdf.splitTextToSize(
            `${t("pdf.leftStatement")}: ${q.left}`,
            textWidth
          ) as string[];
          const selLabelLines = pdf.splitTextToSize(
            `${t("pdf.yourSelection")}: ${q.selectionLabel}`,
            textWidth
          ) as string[];
    ...
    const drawHeaderAndContent = (withLogo: boolean) => {
      ...
      pdf.text(t("pdf.headerTitle"), headerX, 22);
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(9);
      pdf.text(t("pdf.headerSubtitle"), headerX, 34);

      drawSummaryAndTable();
      drawSelectedAnswers();
      pdf.save(t("pdf.fileName"));
    };
  ...
  const handleEmail = () => {
    ...
    const subject = encodeURIComponent(t("share.reportTitle"));
    const bodyLines = [
      t("share.emailBodyLine1"),
      "",
      t("share.emailBodyLine2"),
      shortUrl,
    ];
          ...
          <div className="flex flex-wrap items-center gap-3">
            {mode !== "shared" && (
              <>
                ...
                >
                  {copied
                    ? t("results.linkCopied")
                    : shareDisabled
                    ? t("results.generatingLink")
                    : t("results.copyLink")}
                </button>
                <button
                  type="button"
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-300">
                <span>{t("results.readinessScore")}</span>
      ...
      <footer className="mt-8 flex flex-col items-center justify-center gap-2 border-t border-slate-200 pt-4 text-slate-600 dark:border-slate-700 dark:text-slate-300">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <NextImage
              src="/LeadAI.webp"
              alt={t("results.logoAlt")}
              width={80}
              height={22}
            />
          </a>
          <span className="text-xs">{t("results.copyright")}</span>
        </div>
      </footer>
    </div>
  );
