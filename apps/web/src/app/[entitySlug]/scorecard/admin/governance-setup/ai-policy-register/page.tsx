...
import PolicyManagerPage from "@/app/scorecard/admin/trustops/policies/page";
import { findEntityBySlug } from "@/lib/entityValidation";

export default async function EntityGovernancePolicyRegisterPage({
  params,
}: {
  ...
  if (!entity) {
    ...
    if (!res.ok && res.status === 401) {
      redirect(
        "/register?callbackUrl=" + encodeURIComponent(`/${encodeURIComponent(entitySlug)}/scorecard/admin/governance-setup/ai-policy-register`)
      );
    }
    redirect("/scorecard/admin/governance-setup/ai-policy-register");
  }

  return <PolicyManagerPage entityId={entity.entity_id} entitySlug={entity.slug} />;
}
