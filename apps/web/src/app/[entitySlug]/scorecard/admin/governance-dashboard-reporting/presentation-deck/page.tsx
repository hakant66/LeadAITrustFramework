...
import PresentationDeckPage from "@/app/scorecard/admin/governance-dashboard-reporting/PresentationDeckPage";

type UserEntity = { entity_id: string; role: string; name: string; slug: string; status: string | null };

export default async function EntityPresentationDeckPage({
  params,
}: {
    ...
    if (res.status === 401) {
      redirect(
        "/register?callbackUrl=" +
          encodeURIComponent(`/${encodeURIComponent(entitySlug)}/scorecard/admin/governance-dashboard-reporting/presentation-deck`)
      );
    }
  ...
  if (!entity) {
    ...
    if (first) {
      redirect(`/${encodeURIComponent(first.slug)}/scorecard/admin/governance-dashboard-reporting/presentation-deck`);
    }
    redirect("/scorecard/admin/governance-dashboard-reporting");
  }

  return (
    <PresentationDeckPage
