...
import EmailSettingsPage from "@/app/admin/email-settings/page";
import { findEntityBySlug } from "@/lib/entityValidation";

export default async function EntityEmailSettingsPage({
  params,
}: {
  ...
  if (!entity) {
    ...
    if (res.ok) {
      ...
      if (first) {
        redirect(`/${encodeURIComponent(first.slug)}/admin/email-settings`);
      }
    }
    if (!res.ok && res.status === 401) {
      redirect(
        "/register?callbackUrl=" + encodeURIComponent(`/${encodeURIComponent(entitySlug)}/admin/email-settings`)
      );
    }
