  ...
  return (
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
      {items.map((r) => {
        ...
        const href =
          projectSlug && r.key
            ? `/scorecard/${encodeURIComponent(projectSlug)}/kpis/${encodeURIComponent(
                r.key
              )}`
            : null;
          ...
          <div
            className={`p-2 rounded-xl border border-slate-200 bg-white shadow-sm
                       text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50 ${
                         href ? "hover:border-indigo-300 hover:shadow-md transition" : ""
                       }`}
          >
            {/* Title */}
        ...
        return href ? (
          <button
            ...
            type="button"
            onClick={() =>
              window.open(href, "_blank", "noopener,noreferrer,width=1100,height=780")
            }
          ...
          <div key={r.kpi_id || r.key}>{content}</div>
        );
      })}
    </div>
  );
