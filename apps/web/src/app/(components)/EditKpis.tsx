...
export default function EditKpis({
  ...
  const apiBase = coreApiBase();

  const router = useRouter();
  const [effectiveEntityId, setEffectiveEntityId] = useState<string | undefined>(entityId);
  ...
  useEffect(() => {
    ...
    const cached = sessionStorage.getItem("entityId");
    if (cached) {
      setEffectiveEntityId(cached);
      return;
    }
    (async () => {
      try {
        ...
        const resolved = first?.entity_id ?? first?.id ?? null;
        if (resolved) {
          sessionStorage.setItem("entityId", resolved);
          setEffectiveEntityId(resolved);
        }
      } catch {
        // ignore
      }
  ...
  const handleUpdate = async (k: string) => {
    ...
    try {
      ...
      const url = `${apiBase}/scorecard/${encodeURIComponent(projectId)}${
        effectiveEntityId ? `?entity_id=${encodeURIComponent(effectiveEntityId)}` : ""
      }`;
      const bodies = [
      ...
      try {
        const cid = await resolveControlId(projectId, k, effectiveEntityId);
  ...
  async function safeResolveControlId(kpiKey: string): Promise<string | null> {
    ...
    try {
      const cid = await resolveControlId(projectId, kpiKey, effectiveEntityId);
      if (cid) {
  ...
  const [downloadBusyId, setDownloadBusyId] = useState<string | number | null>(null);
  const preloadedEvidenceKeys = useRef<Set<string>>(new Set());
  ...
  async function loadEvidence(kpiKey: string) {
    ...
    try {
      ...
      const res = await listEvidence(projectId, cid, effectiveEntityId); // normalized to { items: [...] }
  ...
  useEffect(() => {
    let cancelled = false;
    const preloadAllEvidence = async () => {
      const keys = list.map((k) => k.key).filter(Boolean);
      for (const key of keys) {
        if (cancelled) return;
        if (preloadedEvidenceKeys.current.has(key)) continue;
        preloadedEvidenceKeys.current.add(key);
        await loadEvidence(key);
      }
    };
    void preloadAllEvidence();
    return () => {
      cancelled = true;
    };
  }, [list]);
  ...
  async function handleFileSelected(k: Kpi, inputEl: HTMLInputElement) {
    ...
    try {
      ...
      await uploadEvidenceFile(projectId, cid, file, sha, createdBy ?? undefined, effectiveEntityId);
  ...
  async function handleDeleteEvidence(item: any, kpiKey: string) {
    const evId = item?.id ?? item?.evidence_id ?? item?.uid;
    if (!evId) {
      alert("Evidence item is missing an ID.");
      return;
    }
    if (!confirm("Delete this evidence?")) return;
    try {
      await deleteEvidence(evId);
      if (openKey === kpiKey) await loadEvidence(kpiKey);
      router.refresh();
    } catch (err: any) {
  ...
  async function handleOpenEvidence(item: any) {
    const evId = item?.id ?? item?.evidence_id ?? item?.uid;
    if (!evId) {
      alert("Evidence item is missing an ID.");
      return;
    ...
    setDownloadBusyId(evId);
    try {
      const url = await getDownloadUrl(evId);
      if (!popup) {
            ...
            <div className="flex-1 min-w-[280px]">
              <div className="text-sm font-semibold leading-5 break-words text-slate-900 dark:text-slate-50">
                <button
                  type="button"
                  onClick={() => {
                    const base = entitySlug
                      ? `/${encodeURIComponent(entitySlug)}/scorecard/${encodeURIComponent(projectId)}`
                      : `/scorecard/${encodeURIComponent(projectId)}`;
                    const path = `${base}/controls/${encodeURIComponent(k.key)}`;
                    window.open(path, "_blank", "noopener,noreferrer");
                  }}
                  className="text-left hover:underline hover:text-indigo-600 dark:hover:text-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-500 rounded"
                  title={`Open evidence capture for ${k.key}`}
                >
                  {k.name}
                </button>
              </div>
                        ...
                        {items.map((it: any) => (
                          ...
                          >
                            <div className="min-w-0">
                              <div className="text-[12px] truncate text-slate-900 dark:text-slate-50">
                                <button
                                  ...
                                  disabled={downloadBusyId === (it.id ?? it.evidence_id ?? it.uid)}
                                  title="Open evidence"
                                >
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                className="text-[11px] px-2 py-1 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-800 disabled:opacity-50"
                                onClick={() => handleOpenEvidence(it)}
                                disabled={
                                  downloadBusyId === (it.id ?? it.evidence_id ?? it.uid)
                                }
                              >
                                {downloadBusyId === (it.id ?? it.evidence_id ?? it.uid)
                                  ? "Opening..."
                              ...
                              <button
                                type="button"
                                className="text-[11px] px-2 py-1 rounded-lg border border-red-300 dark:border-red-500/60 bg-white dark:bg-slate-900 text-red-700 dark:text-red-200 hover:bg-red-50 dark:hover:bg-red-900/40"
                                onClick={() => handleDeleteEvidence(it, k.key)}
                              >
                                Delete
                              </button>
                            </div>
